...
for i = 1:num_solutions
    ...
    fitness = py.calculate_fitness_model3.calculate_fitness_wrapper(solution, params);
...
% 调用模拟退火萤火虫算法文件
optimal_solution = py.simulated_annealing_firefly_model3.run_simulated_annealing_firefly(initial_solutions, params, T_0, alpha);
...
% 计算当前最优解的适应度值
current_fitness = py.calculate_fitness_model3.calculate_fitness_wrapper(optimal_solution, params);
...
filename = sprintf('../实验数据/model3/result_model3_生产量_%d_管道运输距离_%d_时间_%s.xlsx', production_quantity_final, distance_pipeline_final, char(currentTime));
...
% 获取当前时间并格式化为字符串
optimal_filename = sprintf('../实验数据/model3/optimal_solutions/生产量_%d_管道运输距离_%d_时间_%s.xlsx', production_quantity_final, distance_pipeline_final, char(currentTime));
